...
| 기능 | 설명 |
|------|------|
| 액션 알림 (최상단) | 빨간색(즉시 조치) · 노란색(점검 필요) — 만기 임박 / 연금 1,500만 한도 80%+·초과 / 계획 성공 확률 85%↓ / 인출률 초과 / 비상자금 부족 / 리밸런싱 편차 / 위험점수 / 세후수익률 |
| 핵심 KPI 6종 (시급성 순) | ① 총 순자산(금융+실물) ② 계획 성공 확률(몬테카를로 500경로, 수익률 우선순위·주택연금 설정을 연금 계획 페이지와 공유) ③ 이달 인출·실적 인출률 ④ 올해 수입 합계(이자·배당, 연도 자동 갱신) ⑤ 금융소득(2,000만 대비) ⑥ 비상자금 개월 수 — 클릭 시 해당 페이지로 이동 |
...
| 주택연금 상태 유지 | 활성화 여부·주택가액·가입나이·지급방식을 서버(`config.plan.home_pension`)에 저장 — 끄기 전까지 기기·브라우저 간에도 동일하게 유지, 대시보드 계획 성공 확률과 공유 |
...
| 몬테카를로 시뮬레이션 | 1,000개 수익률 경로(N(μ,σ), σ는 버킷 구성 가중) — 95세 자산 유지 확률·상속 목표 달성 확률·고갈 확률 KPI. 수익률은 저장된 목표 수익률 → 실현 수익률 → 추정 수익률 순으로 사용 |
| 백분위 밴드 차트 | 잔액 차트에 5~95% / 25~75% 음영 + 시뮬레이션 중앙값을 고정 수익률 곡선과 겹쳐 표시 |
...
│   │   ├── simulation.py              # 몬테카를로 연금 자산 시뮬레이션 (numpy)
...
│   │   ├── test_simulation.py         # 몬테카를로 순수 함수 테스트 (시드 고정)
│   │   ├── test_simulation_api.py     # 몬테카를로 라우트 스모크 테스트
