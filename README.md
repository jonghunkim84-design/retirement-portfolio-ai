...
| 핵심 KPI 6종 (시급성 순) | ① 총 순자산(금융+실물) ② 계획 성공 확률(몬테카를로 500경로) ③ 이달 인출·실적 인출률 ④ 연금 한도 사용률 ⑤ 금융소득(2,000만 대비) ⑥ 비상자금 개월 수 — 클릭 시 해당 페이지로 이동 |
...
| 몬테카를로 시뮬레이션 | 1,000개 수익률 경로(N(μ,σ), σ는 버킷 구성 가중) — 95세 자산 유지 확률·상속 목표 달성 확률·고갈 확률 KPI |
