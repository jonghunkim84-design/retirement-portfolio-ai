...
| 기능 | 설명 |
|------|------|
| 지출 입력 | 날짜·금액·카테고리·메모 입력 |
| 월별 지출 차트 | 월별 지출 추이 시각화 |
| 카테고리별 집계 | 항목별 지출 비중 분석 |

#### 인출 관리 (`/withdrawal`)
| 기능 | 설명 |
|------|------|
| 월간 권장 인출액 | 총자산 기반 4% 룰 계산 |
| 연간 인출률 | 실제 인출 기록 기반 인출률 % |
...
| 기능 | 설명 |
|------|------|
| 금융소득 종합과세 모니터 | 연간 금융소득(이자+배당) 누적액 vs 한도 비율 |
...
│   │   ├── risk.py                    # 위험 점수 계산
│   │   ├── withdrawal.py              # 인출 계획 (4% 룰)
│   │   ├── withdrawals.py             # 인출 기록 CRUD
