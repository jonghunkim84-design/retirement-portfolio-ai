...
const STRATEGIES = [
  ...
  },
  {
    ...
    title: '연금 수령액 분산',
    rows: [
      { label: '1,500만원 기준', value: '이하 시 분리과세로 종결' },
      { label: '나이별 세율',    value: '55~69세 5.5% / 70~79세 4.4% / 80세+ 3.3%' },
    ],
    tip: '연금저축·IRP 수령액을 연간 1,500만원 이하로 조정하면 종합소득 합산을 피할 수 있습니다.',
    warning: '국민연금은 공적연금으로 별도 계산됩니다.',
  },
]
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <div className="text-center">
              <p className="text-[11px] text-gray-400 mb-0.5">기준 금액</p>
              <p className="text-lg font-bold text-gray-700">{thresholdLabel}</p>
            </div>
            <div className="text-center">
...
export default function TaxOptimization() {
  ...
  // 금융소득
  const finYtd       = data?.financial_income_ytd ?? 0
  const finRemaining = data?.remaining            ?? FINANCIAL_THRESHOLD
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">{year}년 금융소득 · 연금소득 모니터 및 절세 전략</p>
      </div>
      ...
      <MonitorCard
        ...
        pct={finPct}
        status={finStatus}
        thresholdLabel="2,000만원"
        isLoading={isLoading}
        statusMessages={{
          safe:    '현재 금융소득이 기준의 60% 미만입니다. 여유 한도를 확인하며 관리하세요.',
          ...
          {STRATEGIES.map(s => (
            <div key={s.title} className="card flex flex-col">
              ...
              <div className="border-t border-gray-100 pt-2.5 space-y-1.5">
                <p className="text-[11px] text-blue-600 leading-relaxed">💡 {s.tip}</p>
                {s.warning && (
                  <p className="text-[11px] text-orange-500 leading-relaxed">⚠️ {s.warning}</p>
                )}
              </div>
            </div>
          ))}
