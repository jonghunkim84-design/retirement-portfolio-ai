...
export default function HealthInsurance() {
  ...
  // 소득 입력 (만원/년)
  const [np, setNp] = useState(0)  // 국민연금
  const [pp, setPp] = useState(0)  // 개인연금·IRP
  const [fi, setFi] = useState(0)  // 이자·배당
  const [ws, setWs] = useState(0)  // 근로·사업
  ...
  const sc1Premium = useMemo(() => {
    ...
    const inc = calcTaxableIncome(np, pp, fi2, ws)
    return calcPremium(getIncomeScore(inc), propertyScore)
  }, [np, pp, fi, ws, propertyScore])

  const sc2Premium = useMemo(() => {
    const np2 = Math.max(0, np - 500)
    const inc = calcTaxableIncome(np2, pp, fi, ws)
    return calcPremium(getIncomeScore(inc), propertyScore)
  }, [np, pp, fi, ws, propertyScore])
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          <KpiCard
            label="판정 소득 합계"
            value={`${dependentIncome.toLocaleString('ko-KR')}만원`}
          />
          <KpiCard
            label="소득 한도"
            value={`${HI_CONST.dependentIncomeLimit.toLocaleString('ko-KR')}만원`}
          />
          <KpiCard
            label="피부양자 가능"
